...
export async function generateStaticParams() {
  ...
  const { data } = await supabase
    ...
    .eq('status', 'published')
    .eq('categories.type', 'san-giao-dich');
  return (data ?? []).map((a: any) => ({ slug: a.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const article = await getVerticalArticleBySlug('san-giao-dich', params.slug);
...
export default async function SanGiaoDichArticlePage({ params }: Props) {
  const article = await getVerticalArticleBySlug('san-giao-dich', params.slug);
  if (!article) notFound();
  ...
  try {
    ...
    console.error('[SanGiaoDichArticlePage] R2 fetch error:', err);
  }
