...
export const metadata: Metadata = {
  title: 'Sàn giao dịch',
  description: 'Kinh nghiệm, đánh giá và tin tức về các sàn giao dịch forex, crypto uy tín.',
};
...
export default async function SanGiaoDichPage({ searchParams }: Props) {
  const currentPage = Math.max(1, Number(searchParams.page) || 1);
  const { articles, totalPages } = await getVerticalArticlesPage('san-giao-dich', currentPage);
        ...
        {articles.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
            {articles.map((article: any) => (
              <Link
                key={article.id}
                href={`/san-giao-dich/${article.slug}`}
